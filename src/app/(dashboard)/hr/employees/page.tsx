...
export default function EmployeesPage() {
  ...
  return (
    <div>
      <PageHeader
        titleEn="People & profiles"
        titleAr="الموظفون والملفات"
        descriptionEn="Directory with deep profiles — documents, history, and permissions will map here."
        descriptionAr="دليل مع ملفات تفصيلية — المستندات والسجل والصلاحيات لاحقاً."
      />

      {roleId === "employee" ? (
        <p className="mb-6 rounded-xl border border-amber-200 bg-amber-50/80 px-4 py-3 text-sm text-amber-900">
          {label(
            "Employee view: directory is read-only in this prototype.",
            "عرض الموظف: الدليل للقراءة فقط في هذا النموذج."
          )}
        </p>
      ) : null}
